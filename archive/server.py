...
from flask import Flask, request, render_template, g, redirect, Response, flash, redirect, url_for
...
@app.route('/vehicle')
def vehicle():
    output = []  # declare and assign the output variable an empty list
    select_query='''
    select *
    from INFORMATION_SCHEMA.COLUMNS
    ...
    context = {'data': output, 'col_name': col_name}

    return render_template("tablebase.html", **context)
...
def manufacturer():
    ...
    return render_template("manufacturer.html", **context)
...
def dealership():
    ...
    select_query = "SELECT * from dealership"
    cursor = g.conn.execute(text(select_query))
...
if __name__ == "__main__":
	...
	@click.argument('PORT', default=8123, type=int)
